...
def gate_centering():
	...
	for value in range(90,91):
		...
		# Find orange colors
		mask_vertical_bar = cv2.inRange(combine_blurred, (130,125, 90), (160, 155, 255))
		...
		if(len(vertical_bar_boxes) >= 3):
			...
			cv2.circle(img, mid_bar_center, 5, (0,255,0), thickness=5)
			cv2.circle(img, left_bar_center, 5, (0,255,0), thickness=5)
			cv2.circle(img, right_bar_center, 5, (0,255,0), thickness=5)
			...
			cv2.circle(img, target, 5, (0,0,255), thickness=5)

			cv2.drawContours(img,[mid_bar_box],0,(0,255,0),2)
			cv2.drawContours(img,[right_bar_box],0,(0,255,0),2)
			cv2.drawContours(img,[left_bar_box],0,(0,255,0),2)
			...
			cv2.arrowedLine(img, __FRAME_CENTER, target, 255 , 2)
		else:
			for box in vertical_bar_boxes:
				center = box_center(box)
				cv2.circle(img, center, 5, (0,255,255), thickness=5)
