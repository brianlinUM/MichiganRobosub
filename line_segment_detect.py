...
class HoughBundler:
    ...
    def cut_top(self, horizontal_list, vertical_list, average_bar_height):
        global mid
        slack = 20
        new_vertical_list = []
    ...
    def cluster_vertical_lines(self, vertical_centers):
    ...
    def display_lines(self, frame, horizontal_list, vertical_lines, line_width=2):
        global mid
        line_image = np.zeros_like(frame)
        '''
        mid_points = centering.calc_mid_points(vertical_lines, x_only=1)
        labels = self.cluster_vertical_lines(mid_points)#
...
def main():
    ...
    for value in range(0,105):
        ...
        horizontal_list, vertical_list, average_bar_height = HB.filter_line_segments(line_segs)
        cut_vertical_list = HB.cut_top(horizontal_list, vertical_list, average_bar_height)
